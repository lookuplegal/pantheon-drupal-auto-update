# Pantheon Drupal 7 Auto Update #

## Description ##
Scalable automated testing and deployment of upstream (Drupal 7 core), module and theme updates on [Pantheon](https://pantheon.io) sites with help from [Terminus](https://github.com/pantheon-systems/terminus), [CircleCI](https://circleci.com), [Drush](https://www.drush.org/), [BackstopJS](https://garris.github.io/BackstopJS/) and [Slack](https://slack.com/).
...
5. Check for and apply [Pantheon upstream updates](https://pantheon.io/docs/upstream-updates/)
	* Drupal core updates are managed in the default upstream
...
7. Check for and apply Drupal module updates via [Drush](https://www.drush.org/), if available
8. Check for and apply Drupal theme updates via [Drush](https://www.drush.org/), if available
	* If no Drupal updates are available the script will stop here
...
4. Add [environment variables to CircleCI](https://circleci.com/docs/environment-variables/) for the following:
	* `MULTIDEV`: The multidev name to use for applying/testing updates. Defaults to `update-dr`
	...
	* `SITE_UUID`: The Pantheon site UUID if not provided via `sites-to-auto-update.json`
	* `SITE_NAME`: The Pantheon site name if not provided via `sites-to-auto-update.json`
5. Add an (public) [SSH key to Pantheon](https://pantheon.io/docs/ssh-keys/) and (the private key) [to the CircleCI project](https://circleci.com/docs/permissions-and-access-during-deployment/)
...
## Credits ##
Forked from the work of [Andrew Taylor](https://github.com/ataylorme/wordpress-at-scale-auto-update/) whose project provides a means of automatically updating both WordPress and Drupal.